...
int main(int argc, char * argv[]) {
    ...
    if (DEBUG_RESULT_OUTPUT) {
        printf("\nDebug A[%d;%d] - A[%d;%d]:\n", DEBUG_I, DEBUG_J + 1, DEBUG_I, DEBUG_J + 10);
        i = DEBUG_I;
        for (j = DEBUG_J + 1; j < DEBUG_J + 11 && j < n; j++) {
    ...
    // Create a context  
    context = clCreateContext(0, 2, device_list, NULL, NULL, & err);
	...
	if(COMPARE_TO_SEQUENTIAL){	
		// Create the sequential kernel for comparison
		seq_kernel = clCreateKernel(program, "seq_mat_comp", & err);
	}
	...
	if(COMPARE_TO_SEQUENTIAL){
		...
		size_t seq_global_size[2] = {1,1};
		
		printf(">>> Starting sequential calculation\n");
	
		// Start the timing
		gettimeofday( & Tvalue, & dummy);
		double seq_starttime = (double) Tvalue.tv_sec + 1.0e-6 * ((double) Tvalue.tv_usec);
		
		err = clSetKernelArg(seq_kernel, 0, sizeof(cl_mem), & d_a);
		err |= clSetKernelArg(seq_kernel, 1, sizeof(cl_mem), & d_b);
		err |= clSetKernelArg(seq_kernel, 2, sizeof(cl_mem), & d_c);
		err |= clSetKernelArg(seq_kernel, 3, sizeof(unsigned int), & n);
		unsigned int seq_iter = ITERATION;
		err |= clSetKernelArg(seq_kernel, 4, sizeof(unsigned int), & seq_iter);

		err = clEnqueueNDRangeKernel(queue, seq_kernel, 2, NULL, seq_global_size, seq_local_size,
			0, NULL, NULL);

		if (err != CL_SUCCESS) {
			printf("Error at seq clEnqueueNDRangeKernel(), error code: %d\n", err);
			return 1;
		}
		...
		gettimeofday( & Tvalue, & dummy);
		double seq_endtime = (double) Tvalue.tv_sec + 1.0e-6 * ((double) Tvalue.tv_usec);
		double seq_runtime = (seq_endtime - seq_starttime);
		printf(">>> Sequential execution done: took %.5lf seconds runtime\n", seq_runtime);
	}
